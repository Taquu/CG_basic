...
void display() {
	...
	glLoadIdentity();
	gluLookAt(g_EyeX, g_EyeY, g_EyeZ, 0.0, 0.0, 0.0, 0.5, 1.0, 0.3);
	...
	for (int i = 0; i < g_NumTeapots; i++) {
		const float thetaDegree = deltaTheta * i + 5*g_RotationDegree + 2*sin(g_RotationDegree); // ティーポットの位置を決めるための角度
		...
		// ティーポットの高さ方向の値
		const float y1Pos = g_HeightOffset*2.0+sin(g_RotationDegree); // ★この値を少しずつ変化させることでティーポットが上下に移動する
		const float y2Pos = g_HeightOffset*3.0+cos(g_RotationDegree);
		const float y3Pos = g_HeightOffset*6.0+sin(g_RotationDegree/2);
		const float y4Pos = g_HeightOffset*8.0+cos(g_RotationDegree/2);
		// ティーポットの色の指定
		...
		glPushMatrix();
		if(i%4==0){
			glTranslatef(xPos, y1Pos, zPos);
		}else if(i%4==1){
			glTranslatef(xPos, y2Pos, zPos);
		}else if(i%4==2){
			glTranslatef(xPos, y3Pos, zPos);
		}else{
			glTranslatef(xPos, y4Pos, zPos);
		}
		...
		if(i%4==0){
			...
			displayCylinder(0.3f, y1Pos + 1.f, 32);
		}else if(i%4==1){
			glTranslatef(xPos, -1.f, zPos);
			displayCylinder(0.3f, y2Pos + 1.f, 32);
		}else if(i%4==2){
			glTranslatef(xPos, -1.f, zPos);
			displayCylinder(0.3f, y3Pos + 1.f, 32);
		}else{
			glTranslatef(xPos, -1.f, zPos);
			displayCylinder(0.3f, y4Pos + 1.f, 32);
		}
		glPopMatrix();
	}
...
void init() {
	...
	float lightDiffuseColor0[] = { 0.3f, 0.3f, 0.3f, 0.0f };
	float lightSpecularColor0[] = { 0.8f, 0.8f, 0.8f, 0.0f };
	float lightPosition0[] = { 10.0f, 50.0f, 60.0f, 0.0f };
...
void timer(int val) {
	...
	g_EyeX = g_EyeCenterX + 20*sin(g_RotationDegree/3.0);
	g_EyeY = g_EyeCenterY + 10*cos(g_RotationDegree/3.0);
	g_EyeZ = g_EyeCenterZ/2.0 + 3*sin(g_RotationDegree/5.0) ;
